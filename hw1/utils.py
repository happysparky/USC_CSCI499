...
def stem_data(data):
    ...
    for episode in data:
        ...
        for inst, _ in episode:
            inst = preprocess_string(inst)
            # convert the word to its stem
            inst = [porterStemmer.stem(word) for word in inst.lower().split()]
            inst = " ".join(inst)
            stemmed_inst.append([inst, _])
        stemmed_data.append(stemmed_inst)

    # Write out some of the stemmed instruction examples
    # f = open("experiments/stemmedExamples.txt", "w")
    # f.write(str(stemmed_data[0]))
    # f.write("\n")
    # f.write(str(stemmed_data[1]))
    # f.write("\n")
    # f.write(str(stemmed_data[2]))
    # f.close()
